...
def test_successful_initialization() -> None:
    ...
    # OUGraph can be created
    ou_graph = OUGraph()
    assert isinstance(ou_graph, OUGraph)
    # All internal lists must be initialized with empty values
    for _, list_value in ou_graph.__dict__.items():
...
#     graph = Graph()
#
#     inst_id = OntoUML.name
#     invalid_type = OntoUML.Class
#     graph.add((inst_id, RDF.type, invalid_type))
#
#     try:
#         ou_graph = OUGraph(graph, include_concrete=False)
...
#
#     inst_id = URIRef(EXAMPLE_NS + "name")
#     invalid_type = URIRef(OntoUML.get_namespace() + type_name)
#     graph.add((inst_id, RDF.type, invalid_type))
#
#     try:
