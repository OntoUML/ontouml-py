...
@pytest.mark.parametrize(
    ...
    [
        (None, OntologyRepresentationStyle.ONTOUML_STYLE, 
         {"namespace": None, "representation_style": OntologyRepresentationStyle.ONTOUML_STYLE}),
        ("http://example.com", OntologyRepresentationStyle.UFO_STYLE, 
         {"namespace": "http://example.com", "representation_style": OntologyRepresentationStyle.UFO_STYLE})
    ]
)
def test_project_initialization(namespace: str, representation_style: OntologyRepresentationStyle, expected_values: dict) -> None:
    ...
    assert all(getattr(project, attr) == value for attr, value in expected_values.items()), \
...
def test_project_bibliographic_citations_initialization(bibliographic_citations: set[str], expected_citations: set[str]) -> None:
    ...
    assert project.bibliographic_citations == expected_citations, "Project bibliographic citations should match the expected values"
